...
export default function NavBar() {
    let {selectedItem , modifySelectedItem} = useContext(appContext)
    let [showMenu , setShowMenu] = useState(false);
    let isOpen = showMenu ? "sm:absolute" : "sm:hidden";
    let active = "border-b-[3px] rounded-[2px]"
    return (
        <nav className='py-3 px-[50px] h-14 flex top-0 justify-between fixed w-full bg-color1 items-center z-50 sm:justify-between sm:px-[20px] '>
            <h1 className='text-xl text-white sm:text-[1rem] font-medium'>Abdelkoddous MOUNIR</h1>
            <ul className={`flex gap-7 capitalize text-[1rem] items-center justify-center rounded-b-2xl text-white sm:bg-color1 sm:h-[270px] sm:w-[200px]  sm:flex-col sm:top-14 sm:right-0 ${isOpen}`}>
